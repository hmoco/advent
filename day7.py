...
akwdkl (32)
'''
p = [n for n in p.split('\n') if len(n)]
import re
weights = {}
parents = {}
children = {}
for x in p:
    split_ = re.sub('[\s+]', '', x).split('(')
    weights[split_[0]] = int(split_[1].split(')')[0])
    children[split_[0]] = []
    if '->' in x:
        sp = x.split('->')
        new = re.sub('[\s+]', '', sp[1])
        for child in new.split(','):
            parents[child] = split_[0]
            children[split_[0]].append(child)


def get_child_weight(node):
    if len(children[node]):
        weights_ = []
        for child in children[node]:
            weights_.append(get_child_weight(child))
        if len(set(weights_)) != 1:
            for child in children[node]:
                print 'CHILD: {}, WEIGHT: {}'.format(child, get_child_weight(child))
        else:
            return sum(weights_) + weights[node]
    return weights[node]
...
#     split_ = x.split('->')
#     if len(split_) == 2:
#         potentials.append(re.sub('[\s+]', '', split_[0]).split('(')[0])
#         new = re.sub('[\s+]', '', split_[1])
#         for g in new.split(','):
#             used.append(g)
#
#
