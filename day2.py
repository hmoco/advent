sequence = '''121	59	141	21	120	67	58	49	22	46	56	112	53	111	104	130
...
for r in range(0, row):
    start = r*col
    partial = ar[start:start+col]
    print partial
    # PART 1
    # total = total + (max(partial) - min(partial))
    for x in partial:
        partial.remove(x)
        for t in partial:
            if x%t == 0:
            ...
            if t%x == 0:
                total = total + t/x
        #wtf at some point I got the answer and managed to lose it because proper use of version control is only for smart people
print total
